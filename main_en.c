...
int flash(char*path, char*name, int file_num)
{
	...
	if ((size > MIN_VALID_FILE_SIZE-1) && (size < MAX_VALID_FILE_SIZE+1))
	{
		...
		if (file >= 0)
		{
			...
			printf(">> Error %08x when writing file!\n", file);
		sceIoClose(file);
	}
	else
		printf(">> Error %08x when reading file!\n", size);
...
void menu(void)
{
	...
	printf(" %c & %c - select file, current: ", '<', '>');
	x = pspDebugScreenGetX();
	pspDebugScreenSetTextColor(0xFFFF00FF); // сиреневый
	printf("slot #%i (v%c.%c%c)\n", 1, versions[0], versions[1], versions[2]);
	pspDebugScreenSetTextColor(0xFFFF0000); // синий
	printf(" %c & %c - select path, current: ", 'v', '^');
	pspDebugScreenSetTextColor(0xFFFF00FF); // сиреневый
	printf("%s\n", dest_path_ms0[0]);
	pspDebugScreenSetTextColor(0xFFFF0000); // синий
	printf(" \6 & \5 - select name, current: ");
	pspDebugScreenSetTextColor(0xFFFF00FF); // сиреневый
	printf("%s\n\n", dest_name_all[0]);
	pspDebugScreenSetTextColor(0xFF00FF00); // зелёный
	printf(" \3 - flash file");
	...
	while (1)
	{
		...
		else if (Key.Buttons & PSP_CTRL_LEFT)
		{
			...
			pspDebugScreenSetXY(x + 0, y + 0);
			pspDebugScreenSetTextColor(0xFFFF00FF); // сиреневый
			printf("slot #%i (v%c.%c%c)", i1+1, versions[0+3*i1], versions[1+3*i1], versions[2+3*i1]);
		}
...
void print(void)
{
	...
	for (i = 0; i < 4; i++)
	{
		...
		if ((size > MIN_VALID_FILE_SIZE-1) && (size < MAX_VALID_FILE_SIZE+1))
		{
			...
			printf(">> version%i.txt not found =(\n", i+1);
		printf("\n");
	}
...
void check_gen_hack(void)
{
	if (sceKernelDevkitVersion() == 0x05050010)
	{
		...
		if ((buf[0x8] == '9') && (buf[0xA] == '9'))
		{
			pspDebugScreenClear();
			pspDebugScreenSetTextColor(DEFAULT_TEXT_COLOR);
			printf("VERSiON.TXT SWiTCHER v%01i.%01i by Yoti\n", MAJOR_VER, MINOR_VER);
			printf("\n");
			pspDebugScreenSetTextColor(0xFF0000FF); // красный
			printf(" WARNING!!!\n");
			pspDebugScreenSetTextColor(DEFAULT_TEXT_COLOR);
			printf("Program detected TEAM GEN version.txt patch.\n");
			printf("It's recommended to restore it back to 5.50.\n");
			printf("\n");
			printf(" \3 - please restore it now\n");
			printf(" \4 - skip this time\n");
			printf(" \5 - skip & never ask again\n");
